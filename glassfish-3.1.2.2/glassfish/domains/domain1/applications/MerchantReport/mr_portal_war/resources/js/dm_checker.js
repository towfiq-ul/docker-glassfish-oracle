...
function loadJTableChangeRequestData()
{
    $('#jtableFinChecker').jtable({
        ...
        selectOnRowClick: true, //Enable this to only select using checkboxes
        actions: {
            listAction: 'https://' + window.location.host + '/mr_portal/api/merchantreport/dm_temp_masterdata_checker'
        },
        fields: {
            merAcc: {
                title: 'MerchantAccount',
...
//    $("#append_md").prop('disabled', true);
    ...
    if (selectedRows.length > 0) {
        ...
        //
        $.ajax({
            url: 'https://' + window.location.host + '/mr_portal/api/merchantreport/dm_approve',
...
//    $("#append_md").prop('disabled', true);
    ...
    if (selectedRows.length > 0) {
        ...
        //
        $.ajax({
            url: 'https://' + window.location.host + '/mr_portal/api/merchantreport/dm_reject',
            contentType: 'application/json; charset=UTF-8', //Sending Type
            dataType: 'text', //Accept Type
            type: 'POST',
            data: JSON.stringify(finalJson),
